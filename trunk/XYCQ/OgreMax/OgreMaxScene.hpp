...
{   
    ...
    public:
		...
		/*	init PG engine */
		inline void initPG(Camera *cam, Real wsz)
		{
			mPGMgr->initialise(cam,wsz);
		}
		inline void initPGTrees(Real psz, bool autoset = false)
		{
			mPGMgr->initTrees(psz,autoset);
		}
		inline void initPGBushes(Real psz, bool autoset = false)
		{
			mPGMgr->initBushes(psz,autoset);
		}
		inline void initPGGrass(Real psz, bool autoset = false)
		{
			mPGMgr->initGrass(psz,autoset);
		}

		/*	set PG tree	*/
		inline void setPGTree(Ogre::Vector2 &bat,Ogre::Vector2 &imp)
		{
		...
		/*	set PG grass	*/
		inline void setPGGrass(Real dist)
		{
			mPGMgr->setGrassPage(dist);
			mPGMgr->setGrassPageLoader();
		}
		inline void addPGGrass(String &mat, Vector2 &minsz, Vector2 &maxsz, Real dens,bool anim=false)
		{
			if(mPGMgr->hasInited(PM_GRASS))
				mPGMgr->addGrass(mat,minsz,maxsz,dens,anim);
		}

		/*	set Height function	*/
		inline void setPGHeightFunction(PAGE_MODE pm,
			Real (*hf)(Real x, Real z, void *userData), void *extra = 0)
		{
			mPGMgr->setHeightFunction(pm,hf,extra);
		}

		/*	You can add trees or bushes besides the max XML file.	*/
		inline void addPGTree(Entity *ent, Vector3 &pos, Degree yaw = Degree(0), Real scale = 1.0)
		{
			if(mPGMgr->hasInited(PM_TREE))
				mPGMgr->addTree(ent,pos,yaw,scale);
		}
		inline void addPGBush(Entity *ent, Vector3 &pos, Degree yaw = Degree(0), Real scale = 1.0)
		{
			if(mPGMgr->hasInited(PM_BUSH))
				mPGMgr->addBush(ent,pos,yaw,scale);
		}
		inline void showPG(PAGE_MODE show)
		{
			mPGMgr->show(show);
		}
		inline void setPGCamera(Camera *cam)
		{
			mPGMgr->setCamera(cam);
		}
		//	设置草密度图及各种阴影图
		inline void setPGGrassBounds(Forests::TBounds &bounds)
		{
			mPGMgr->setGrassMapBounds(bounds);
		}
		inline void setPGGrassDensityMap(String &dens)
		{
			mPGMgr->setGrassDensityMap(dens);
		}
		inline void setPGGrassColorMap(String &color)
		{
			mPGMgr->setGrassColorMap(color);
		}
		inline void setPGTreeColorMap(String &map)
		{
			mPGMgr->setTreeColorMap(map);
		}
		inline void setPGBushColorMap(String &map)
		{
			mPGMgr->setBushColorMap(map);
		}
    protected:
        /** 
